...
@section('title', 'GDPR Report — ' . $site->domain)
@section('page-title', 'GDPR Report')
...
<div class="space-y-6">
    ...
    <div class="flex items-start justify-between flex-wrap gap-3">
        <div>
            <nav class="text-sm mb-2">
                ...
                <span class="text-gray-400 mx-2">/</span>
                <span class="text-gray-700">GDPR Report</span>
            </nav>
            <h2 class="text-lg font-semibold text-gray-900">{{ $site->site_name ?: $site->domain }}</h2>
            <p class="text-sm text-gray-500">{{ $report['site']['domain'] }} · Customer: {{ $site->customer->name ?? '—' }}</p>
        </div>
        <div class="flex items-center gap-2">
            @foreach (\App\Services\Compliance\GdprReportService::ALLOWED_PERIODS as $p)
                <a href="{{ route('admin.sites.gdpr-report', ['site' => $site, 'period' => $p]) }}"
            ...
            @endforeach
            <a href="{{ route('admin.sites.gdpr-report', ['site' => $site, 'period' => $period, 'format' => 'json']) }}"
            ...
            <button type="button" onclick="window.print()" class="inline-flex items-center px-3 py-1.5 text-sm rounded-md bg-white border border-gray-300 hover:bg-gray-50 text-gray-700"><i class="fas fa-print mr-1"></i> Print</button>
        </div>
    </div>

    {{-- Period card --}}
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 flex items-start justify-between flex-wrap gap-4">
        <div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5">
            <div class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Sessions</div>
            <div class="mt-1 text-3xl font-bold text-gray-900">{{ number_format($report['summary']['sessions']) }}</div>
        </div>
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5">
            <div class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Total consents</div>
            <div class="mt-1 text-3xl font-bold text-gray-900">{{ number_format($report['summary']['total_consents']) }}</div>
        </div>
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5">
            <div class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Consent rate</div>
            <div class="mt-1 text-3xl font-bold text-gray-900">{{ $report['summary']['consent_rate_pct'] !== null ? $report['summary']['consent_rate_pct'] . '%' : '—' }}</div>
        </div>
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5">
            <div class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Accept-all rate</div>
            <div class="mt-1 text-3xl font-bold text-gray-900">{{ $report['summary']['accept_all_pct'] }}%</div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border {{ $teOk ? 'border-emerald-200' : 'border-red-300' }}">
        ...
        <div class="p-6 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div><div class="text-xs text-gray-500 uppercase tracking-wider">Signed</div><div class="mt-1 text-2xl font-semibold text-gray-900">{{ $te['records_signed'] }}</div></div>
            <div><div class="text-xs text-gray-500 uppercase tracking-wider">Verified</div><div class="mt-1 text-2xl font-semibold text-emerald-700">{{ $te['signatures_verified'] }}</div></div>
            <div><div class="text-xs text-gray-500 uppercase tracking-wider">Unsigned (legacy)</div><div class="mt-1 text-2xl font-semibold text-gray-500">{{ $te['records_unsigned_legacy'] }}</div></div>
            <div><div class="text-xs text-gray-500 uppercase tracking-wider">Failed verify</div><div class="mt-1 text-2xl font-semibold {{ $te['signatures_failed_verify'] > 0 ? 'text-red-700' : 'text-gray-500' }}">{{ $te['signatures_failed_verify'] }}</div></div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="p-6">
            @if (empty($report['cookies']['by_category']))
                <p class="text-sm text-gray-500">No cookies catalogued yet.</p>
            @else
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="p-6 grid grid-cols-2 md:grid-cols-5 gap-4 text-sm">
            <div><div class="text-xs text-gray-500 uppercase tracking-wider">Total</div><div class="mt-1 text-2xl font-semibold text-gray-900">{{ $dsar['total_requests'] }}</div></div>
            <div><div class="text-xs text-gray-500 uppercase tracking-wider">Completed</div><div class="mt-1 text-2xl font-semibold text-emerald-700">{{ $dsar['completed'] }}</div></div>
            <div><div class="text-xs text-gray-500 uppercase tracking-wider">Pending</div><div class="mt-1 text-2xl font-semibold text-amber-700">{{ $dsar['pending'] }}</div></div>
            <div><div class="text-xs text-gray-500 uppercase tracking-wider">Rejected</div><div class="mt-1 text-2xl font-semibold text-gray-500">{{ $dsar['rejected'] }}</div></div>
            <div><div class="text-xs text-gray-500 uppercase tracking-wider">SLA breaches</div><div class="mt-1 text-2xl font-semibold {{ $dsar['sla_breaches'] > 0 ? 'text-red-700' : 'text-gray-500' }}">{{ $dsar['sla_breaches'] }}</div></div>
            <div class="col-span-2 md:col-span-5 pt-2 border-t border-gray-100">
                <div class="text-xs text-gray-500 uppercase tracking-wider">Average response time</div>
                <div class="mt-1 text-sm text-gray-700">{{ $dsar['avg_response_days'] !== null ? $dsar['avg_response_days'] . ' days' : 'No completed requests yet' }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="p-6 grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
            <div><div class="text-xs text-gray-500 uppercase tracking-wider">Retention window</div><div class="mt-1 text-2xl font-semibold text-gray-900">{{ $ret['consent_expiry_days'] }} days</div></div>
            <div><div class="text-xs text-gray-500 uppercase tracking-wider">Records in period</div><div class="mt-1 text-2xl font-semibold text-gray-900">{{ number_format($ret['consent_records_in_period']) }}</div></div>
            <div><div class="text-xs text-gray-500 uppercase tracking-wider">Past expiry, pending purge</div><div class="mt-1 text-2xl font-semibold {{ $ret['records_past_expiry_pending_purge'] > 0 ? 'text-amber-700' : 'text-gray-500' }}">{{ number_format($ret['records_past_expiry_pending_purge']) }}</div></div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <div class="px-6 py-4 border-b border-gray-200">
            ...
            <p class="text-sm text-gray-500">As of report generation.</p>
        </div>
